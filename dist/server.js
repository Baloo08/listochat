...
__export(tenant_almendro_repo_exports, {
  ...
  getElectronicVoucherByKeyOrId: () => getElectronicVoucherByKeyOrId,
...
async function getElectronicVoucherByKeyOrId(keyOrId) {
    ...
    AlmendroService = class {
      ...
      static async emitVoucher(tenantId, params) {
        ...
        try {
          ...
          if (responseBody.errors && typeof responseBody.errors === "object") {
            const detailErrors = Object.entries(responseBody.errors).map(([field, errs]) => `${field}: ${Array.isArray(errs) ? errs.join(", ") : errs}`).join(" | ");
            if (detailErrors) {
      ...
      static async emitOrderInvoice(tenantId, orderId) {
        try {
          ...
          const paymentMethodCode = order.paymentMethod === "card" ? "02" : ["sinpe", "transfer"].includes(order.paymentMethod) ? "04" : "01";
       ...
       */
      static async getVoucherPdf(keyOrId) {
        try {
          ...
          if (!voucher) {
            return { success: false, status: 404, message: "Comprobante electr\xF3nico no encontrado" };
          }
          const config = await getTenantAlmendroConfigRaw(voucher.tenantId);
          if (!config || !config.apiKey || !config.apiKey.trim()) {
            return { success: false, status: 400, message: "Configuraci\xF3n de facturaci\xF3n no disponible para este comercio" };
          }
          ...
          return {
            success: false,
            status: 500,
            message: `Error interno de conexi\xF3n con el proveedor de facturaci\xF3n: ${err.message}`
          };
        }
      }
    };
...
init_evolution();
init_env();
async function getOrCreateSuperadminTenantId() {
...
async function getPlatformTilopayConfig() {
  ...
  try {
    ...
    console.error("[PlatformTilopay] Error al consultar configuraci\xF3n de superadmin:", e);
  }
  return null;
}
...
async function enqueueMessage(tenantId, remoteJid, pushName, cleanPhone, userMessage, instanceName, isVoiceNote = false) {
  ...
  if (duplicateRes.rows.length > 0) {
    console.log(`[Queue] \u{1F6D1} Deduplicated identical message for ${remoteJid} created within last 60s. Skipping duplicate queueing.`);
...
`;
  ...
  Foto oficial: ${pUrl}`;
      ...
      if (store?.acceptTransfer && store.bankAccountInfo) {
        ...
        if (cleanBank.length > 5 && !cleanBank.toLowerCase().includes("tu cuenta") && !cleanBank.toLowerCase().includes("tu banco") && !cleanBank.toLowerCase().includes("tu n\xFAmero")) {
      ...
      if (pMethods.length > 0) paymentText = "\u{1F4B3} M\xE9todos de Pago Disponibles: " + pMethods.join(" | ") + "\n";
      specializedPrompt = `
...
REGLAS DE ORO DE VENTA Y CIERRE DE PEDIDOS (CALIDEZ TICA Y CERO ALUCINACI\xD3N):
...
2. Si el cliente busca o pregunta por un producto que NO est\xE1 en la lista oficial (o si el cat\xE1logo est\xE1 vac\xEDo), TIENES TERMINANTEMENTE PROHIBIDO inventar que lo venden, inventar precios o stock. Debes responder con honestidad y calidez tica (*"Disculpa ${customerFirstName}, en este momento no disponemos de ese art\xEDculo en nuestro cat\xE1logo oficial"*), y sugerirle las opciones que s\xED comercializan.
3. TRATO HUMANO, FRESCO Y NATURAL (CERO ROB\xD3TICO):
   - Dir\xEDgete al cliente por su primer nombre (*${customerFirstName}*) solo de vez en cuando de forma c\xE1lida y humana (*"\xA1Con mucho gusto!", "\xA1Excelente elecci\xF3n!", "\xA1Claro que s\xED!"*).
   - EST\xC1 ESTRICTAMENTE PROHIBIDO repetir el nombre completo del cliente en cada mensaje como un robot.
   - NUNCA uses frases de plantilla r\xEDgidas y vac\xEDas como "Totalizando tus compras" o "Est\xE1s a un click del viaje perfecto".
4. C\xC1LCULO MATEM\xC1TICO EXACTO Y GRAN TOTAL:
   - Cuando el cliente indique cantidades o pregunte por el total, calcula la multiplicaci\xF3n y la suma con total exactitud.
   - Es OBLIGATORIO desglosar los productos y mostrar claramente la l\xEDnea final de suma total:
     \u2022 3 [Producto A]: \u20A1XX,XXX
     \u2022 1 [Producto B]: \u20A1XX,XXX
     *Total a pagar:* \u20A1XX,XXX
5. CIERRE Y EMISI\xD3N DETERMINISTA DE ORDEN (ACCI\xD3N INMEDIATA):
...
Fecha/Hora CR: ${crTime}
${cleanAddress ? `\u{1F4CD} Direcci\xF3n F\xEDsica: ${cleanAddress}
` : ""}
...
4. PUENTE COMERCIAL OPORTUNO: Solo si el cliente realiza una consulta general aislada o saludo inicial, inv\xEDtale amablemente a conocer nuestro cat\xE1logo o servicios. NUNCA hagas esta pregunta si el cliente ya est\xE1 en medio de un pedido, compra o consultando un tema espec\xEDfico.
...
HUMANIZACI\xD3N Y NATURALIDAD CONVERSACIONAL TICA (CERO ROB\xD3TICO):
1. El cliente se llama ${customerFirstName}. Puedes dirigirte a \xE9l por su primer nombre (*${customerFirstName}*) solo de vez en cuando de forma c\xE1lida y fresca.
2. PROHIBICI\xD3N ESTRICTA: EST\xC1 TERMINANTEMENTE PROHIBIDO repetir el nombre completo del cliente ("${rawSenderName}") en cada mensaje como un robot.
3. Comun\xEDcate con calidez, agilidad y frescura costarricense (*pura vida*, con mucho gusto, claro que s\xED, excelente).
4. Prohibido usar frases r\xEDgidas o de plantilla repetitiva como "Totalizando tus compras" o "Est\xE1s a un click del viaje perfecto".
`.trim();
...
Hemos recibido tu comprobante para el pedido *#${pendingOrder.orderNumber}* (\u20A1${Number(pendingOrder.total).toLocaleString("es-CR")}). Nuestro equipo lo verificar\xE1 y confirmaremos tu pedido en breve. \xA1Gracias! \u2705`;
    ...
    if (key.id) {
      ...
      if (existingMsg.rows.length > 0) {
        console.log(`[Webhook] \u{1F6D1} Deduplicated WhatsApp message ${key.id} for tenant '${tenant.name}' (already received). Dropping duplicate event.`);
        return;
      }
    }
    if (fromMe) {
...
router30.get("/platform-config", async (req, res) => {
  try {
    const superadminTenantId = await getOrCreateSuperadminTenantId();
    ...
    if (hasEnvConfig) {
      const maskedKey = process.env.TILOPAY_PLATFORM_KEY.length > 8 ? `${process.env.TILOPAY_PLATFORM_KEY.slice(0, 4)}\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022${process.env.TILOPAY_PLATFORM_KEY.slice(-4)}` : "\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022";
      res.json({
        isConfigured: true,
        isEnabled: true,
    ...
    console.error("[SuperadminBilling] Error al obtener configuraci\xF3n de plataforma:", error);
    res.status(500).json({ error: error.message || "Error al obtener configuraci\xF3n de plataforma" });
  }
});
router30.post("/test-platform-config", async (req, res) => {
  try {
    ...
    if (!testKey || !apiUser || !testPass) {
      res.status(400).json({ success: false, error: "Todos los campos (API Key, Usuario y Contrase\xF1a) son requeridos para la prueba." });
      return;
    }
    ...
    res.status(500).json({ error: error.message || "Error en prueba de conexi\xF3n con Tilopay" });
  }
});
router30.post("/platform-config", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Credenciales de Tilopay de la plataforma guardadas con \xE9xito.",
      config: updated
    });
  } catch (error) {
...
router34.get(["/public/voucher-pdf/:keyOrId", "/public/vouchers/:keyOrId/pdf"], async (req, res) => {
  try {
    ...
    if (!pdfRes.success || !pdfRes.buffer) {
      ...
      if (acceptsHtml) {
        res.status(pdfRes.status || 404).send(`
          ...
          <head>
            ...
            <title>Factura Electr\xF3nica - Betico</title>
            <style>
          ...
          <body>
            <div class="card">
              ...
              <h1>Comprobante Electr\xF3nico no disponible</h1>
              <p>${pdfRes.message || "El comprobante a\xFAn est\xE1 en proceso de emisi\xF3n ante el Ministerio de Hacienda o no se encuentra disponible temporalmente."}</p>
              <a href="javascript:location.reload()" class="btn">Reintentar actualizaci\xF3n</a>
            </div>
          </body>
          </html>
        `);
      ...
      res.status(pdfRes.status || 404).json({
        success: false,
        error: pdfRes.message || "No se pudo obtener el PDF del comprobante electr\xF3nico"
      });
      return;
    }
